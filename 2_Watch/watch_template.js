...
export default class Watch extends THREE.Group {
    constructor(cityName, center = new THREE.Vector2(0.0, 0.0), radius = 0.75, nameBackgroundColor = 0xffffff, nameForegroundColor = 0x000000, dialColor = 0x000000, markersColor = 0xffffff, handsHMColor = 0xffffff, handSColor = 0xff0000) {
        ...
        let startingAngle = 0;
        const angleIncrement = Math.PI/segments;
        
        for(let i = 0; i < segments*2; i++){
            let x = radius * Math.cos(startingAngle);
            let y = radius * Math.sin(startingAngle);
        ...
        points = [new THREE.Vector3(0,0,0), new THREE.Vector3(0.5*radius,0.5*radius,0)];
        geometry = new THREE.BufferGeometry().setFromPoints(points);
        material = new THREE.LineBasicMaterial({color: 0x000000});
        ...
        points = [new THREE.Vector3(0,0,0), new THREE.Vector3(0.7*radius,0.7 *radius,0)];
        geometry = new THREE.BufferGeometry().setFromPoints(points);
    ...
    update() {
        ...
        /* To-do #5 - Compute the minute hand angle. It depends mostly on the current minutes value (time[1]), 
        but you will get a more accurate result if you make it depend on the seconds value (time[2]) as well.
        angle = ...;
        this.handM.rotation.z = angle; */
        
        angle = (Math.PI / 2.0 - 2.0 * Math.PI * (time[1] + time[2]/60.0) / 60.0) - Math.PI/4;
        this.handM.rotation.z = angle;

        /* To-do #6 - Compute the hour hand angle. It depends mainly on the current hours value (time[0]). Nevertheless,
        you will get a much better result if you make it also depend on the minutes and seconds values (time[1] and time[2] respectively).
        angle = ...;
        this.handM.rotation.z = angle;*/

        angle = (Math.PI / 2.0 - 2.0 * Math.PI * (time[0] + time[1]/60.0 + time[2]/3600.0) / 12.0) - Math.PI/4;
